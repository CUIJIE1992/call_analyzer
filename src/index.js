...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname.startsWith('/static/css/')) {
      const cssFile = pathname.replace('/static/css/', '');
      const cssContent = await getCSS(cssFile);
      if (cssContent) {
    ...
    if (pathname.startsWith('/static/js/')) {
      const jsFile = pathname.replace('/static/js/', '');
      const jsContent = await getJS(jsFile);
      if (jsContent) {
    ...
    if (pathname === '/history' || pathname === '/history.html') {
      const html = await getHTML('history');
      return new Response(html, {
        headers: {
          'Content-Type': 'text/html;charset=UTF-8'
        }
...
async function getCSS(filename) {
  ...
  return cssFiles[filename] || null;
}

// JS 文件内容
async function getJS(filename) {
  // 这里可以添加 JS 文件内容
  return null;
}
