...
def generate_pdf_report(analysis, speaker1=None, speaker2=None):
    ...
    if key_info:
        ...
        story.append(Spacer(1, 10))
        story.append(Paragraph(f"<b>客户联系方式：</b>{key_info.get('客户联系方式', '暂无')}", styles['ChineseBody']))
        story.append(Paragraph(f"<b>顾问联系方式：</b>{key_info.get('顾问联系方式', '暂无')}", styles['ChineseBody']))
    ...
    story.append(Paragraph("本报告由购房电话智能分析系统自动生成", styles['ChineseSubtitle']))
    story.append(Paragraph(f"© {datetime.now().year} AI驱动的房产销售洞察与客户跟进策略", styles['ChineseSubtitle']))
